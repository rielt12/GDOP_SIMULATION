function [state rec_GDOP, Error, delta_y_0,error_0,A_0,del_ADR] = doppler_shift_positioning_c(shift, pos, lambda, current_time, ephemeris, JD_prop_to)
...
%rec_pos(2,1) = 0;
%rec_pos(3,1) = 0;
c_rec_clock_bias =c*1e-3;
rec_vel = [10;10;10];
c_rec_clock_bias_rate=10;
...
for i=1:length(shift)
    ...
    del_ADR(i) = acculumulated_delta_range_derivative_c(rec_pos, c_rec_clock_bias,elapsedtime, rec_vel, c_rec_clock_bias_rate, pos(i,:)',lambda,ephemeris(i,1), JD_prop_to);
    R(i,i) = (0.01)^2;
...
for i=1:length(shift)
A(i,:) = Jacobian_Psiaki_Row_Numerical_c(rec_pos, c_rec_clock_bias,elapsedtime, rec_vel, c_rec_clock_bias_rate, pos(i),lambda,ephemeris(i), JD_prop_to);
end
A_0 = A;
vpa(A_0)
...
while(0.5*norm(error)^2 >1e-8 && iter < max_iter)
iter  
0.5*norm(error)^2
...
%y_i(2,1) = y_i(2,1)+(tau/10)*delta_y(2,1);
...
for i=1:length(shift)
 ...
 Error(iter+1,1) = 0.5*norm(error)^2;
